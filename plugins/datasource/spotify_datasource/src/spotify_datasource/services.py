...
def create_graph(auth_token: str, artist_name: str, max_neighbours: int, recursion_depth: int) -> Graph:
    # get artist by name
    # find its relatives recursively
    # create graph
    artist: Artist = find_artist_by_name(artist_name, auth_token)
...
def find_artist_by_name(artist_name: str, auth_token: str) -> Artist:
    try:
...
def find_related_artists(artist: Artist, auth_token: str, max_neighbours: int) -> List[Artist]:
    ...
    try:
        response = requests.get(url="https://api.spotify.com/v1/search",
                                headers={
                                    "Authorization": f"Bearer {auth_token}"},
                                params={"q": f"genre:{artist.genres[0] if artist.genres else 'rap'}",
                                        "type": "artist",
